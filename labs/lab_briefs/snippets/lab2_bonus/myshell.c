...
void my_quit(void) {
    ...
    for (int i=0; i<processes_started; i++) {
        if (process_table[i].process_status == 0) {
            ...
            //  if process is stopped; send SIGTERM to it and wait for it to exit
...
int start(size_t num_tokens, char **tokens) {
    size_t new_num_tokens = num_tokens;
    ...
    if (you_want_concurrent(num_tokens, tokens) == 1) {
        ...
        if (child_pid == 0) {
            ...
            update_process_record(child_pid, 0, WEXITSTATUS(exit_status));
            blocking_pid = -1;
...
void my_info() {
    for (int i=0; i<processes_started; i++) {
        ...
        if (curr_block.process_status == 1) {
            //  if the process is still running
            printf("[%d] Running\n", curr_block.pid);
        } else if (curr_block.process_status == 2) {
            //  if "SIGTERM" has been sent to the process
            printf("[%d] Terminating\n", curr_block.pid);
        } else if (curr_block.process_status == 3) {
            //  if the process has been stopped by "SIGTSTP"
            printf("[%d] Stopped\n", curr_block.pid);
        } else {
...
void my_SIGCHLD_handler(int sig) {
    int exit_status;
    int exited_child_pid = waitpid((pid_t)-1, &exit_status, WNOHANG);
