...
int add_process_record(pid_t process_id);
...
struct PROCESS_CONTROL_BLOCK {
    pid_t pid;
    int process_status; // 0 for "exited", 1 for "running", 2 for "terminating", 3 for "stopped"
    int exit_status; // only initialized when process has exited
    int stat_loc; // the "raw" status code of the process
};
...
/* Global integer that holds the PID of the currently running BLOCKING process */
pid_t blocking_pid = -1; // if value is -1, means there are no BLOCKING processes currently executing
...
void my_process_command(size_t num_tokens, char **tokens) {
    ...
    } else if (command_is("fg", tokens) == 1) {
        my_fg(atoi(tokens[1]));
    } else {
...
void my_quit(void) {
    ...
    for (int i=0; i<processes_started; i++) {
        if (process_table[i].process_status == 0) {
            ...
            killpg(process_table[i].pid, SIGTERM);
            waitpid(process_table[i].pid, NULL, 0);
        } else if (process_table[i].process_status == 2) {
            //  if process is terminating; wait for it to exit
            
            waitpid(process_table[i].pid, NULL, 0);
        } else {
            //  if process is stopped; send SIGTERM to it, followed by SIGCONT and [wait for it to exit]

            killpg(process_table[i].pid, SIGCONT);
            killpg(process_table[i].pid, SIGTERM);
            waitpid(process_table[i].pid, NULL, 0);
        }
    }
...
int start(size_t num_tokens, char **tokens) {
    ...
    if (you_want_concurrent(num_tokens, tokens) == 1) {
        ...
        if (child_pid == 0) {
            ...
            //  SET custom handlers for the signals "SIGTSTP" and "SIGINT" in the PARENT process
        ...
        if (child_pid == 0) {
            ...
            int process_index = add_process_record(child_pid);
            blocking_pid = child_pid;

            // int exit_status;
            //  use WUNTRACED to allow WIFSTOPPED to work (which won't work with regular wait())
            waitpid(child_pid, &process_table[process_index].stat_loc, WUNTRACED);

            //  STOP everything if a program fails
            if (WIFEXITED(process_table[process_index].stat_loc) == 1 && WEXITSTATUS(process_table[process_index].stat_loc) != 0) {
                update_process_record(child_pid, 0, WEXITSTATUS(process_table[process_index].stat_loc));

                return WEXITSTATUS(process_table[process_index].stat_loc);
            } 

            //  shell will continue accepting user commands if it receives SIGINT
            if (WIFSIGNALED(process_table[process_index].stat_loc) == 1 && WTERMSIG(process_table[process_index].stat_loc) == SIGINT) {
                printf("[%d] interrupted\n", child_pid);

                return 0;
            }
            //  shell will continue accepting user commands if it receives SIGTSTP
            if (WIFSTOPPED(process_table[process_index].stat_loc) == 1 && WSTOPSIG(process_table[process_index].stat_loc) == SIGTSTP) {                
                printf("[%d] stopped\n", child_pid);

                update_process_record(child_pid, 3, -1);
...
int add_process_record(pid_t process_id) {
    ...
    int process_index = processes_started;
    processes_started++;
...
void my_info() {
    for (int i=0; i<processes_started; i++) {
        ...
        if (curr_block.process_status == 2) { /* IF a SIGTERM signal was previously sent to the process */
            ...
            if (any_changes == 0) {
                ...
                //  if waitpid(...) returns -1; this means that the process has exited

                printf("[%d] Exited %d\n", curr_block.pid, curr_block.exit_status);
            }
        } else if (curr_block.process_status == 1) { /* IF the process was set to run CONCURRENTLY in the background */
            int curr_status;
            int any_changes = waitpid(process_table[i].pid, &curr_status, WNOHANG);
        ...
        } else if (curr_block.process_status == 3) { /* IF the process was STOPPED by SIGTSTP */
            printf("[%d] Stopped\n", curr_block.pid);
        } else { /* IF the process was marked as EXITED -> curr_block.process_status == 0 */
            printf("[%d] Exited %d\n", curr_block.pid, curr_block.exit_status);
        }
    }
}
    ...
    for (int i=0; i<processes_started; i++) {
        if (process_table[i].pid == process_id) {
            if (process_table[i].process_status == 1) { //  if process is still running
                ...
                //  shell will continue accepting user commands if it receives SIGTSTP
                if (WIFSTOPPED(exit_status) && WSTOPSIG(exit_status) == SIGTSTP) {                
                    printf("[%d] stopped\n", process_id);
                    update_process_record(process_id, 3, -1);
                    return;
                }
...
void my_terminate(pid_t process_id) {
    for (int i=0; i<processes_started; i++) {
        if (process_table[i].pid == process_id) {
            if ((process_table[i].process_status == 1) | (process_table[i].process_status == 3)) { //  if process is still running or is stopped
...
void my_SIGTSTP_handler(int sig) {
    ...
    if (blocking_pid != -1) {
        killpg(getpgid(blocking_pid), SIGTSTP);
    }
...
void my_SIGINT_handler(int sig) {
    ...
    if (blocking_pid != -1) {
        killpg(getpgid(blocking_pid), SIGINT);
    }    

    printf("[%d] interrupted\n", blocking_pid);

    int exit_status;
    waitpid(blocking_pid, &exit_status, 0);

    update_process_record(blocking_pid, 0, WEXITSTATUS(exit_status));
    blocking_pid = -1;
}
...
void my_fg(pid_t process_id) {
    ...
    for (int i=0; i<processes_started; i++) {
        if (process_table[i].pid == process_id) {
            if (process_table[i].process_status == 3) { // if process status is "stopped"
                //  ask process to continue executing
                ...
                //  shell will continue accepting user commands if it receives SIGINT or SIGTSTP
                if (WIFSIGNALED(exit_status) && WTERMSIG(exit_status) == SIGINT) {
                    printf("[%d] interrupted\n", process_id);
                    return;
                }
